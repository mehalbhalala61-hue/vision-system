...
class LRFinder:
    ...
    def run(self, train_loader: DataLoader, cfg: dict) -> float:
        ...
        while step < num_iter:
            ...
            # LR Finder must use HARD labels — NO Mixup/CutMix.
            # SoftCrossEntropyLoss expects (B, C) float — one_hot convert.
            # Bug fix: augmentation during LR finding = noisy curve = wrong LR.
            import torch.nn.functional as F_lr
            num_cls     = logits.size(1)
            soft_labels = F_lr.one_hot(labels, num_cls).float()
